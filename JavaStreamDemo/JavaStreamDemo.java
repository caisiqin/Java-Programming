...
public class JavaStreamDemo {
    public static void main(String[] args) {
        ...
        // Filter
        List<Person> female = people.stream().filter(person -> person.getGender().equals(Gender.FEMALE)).collect(Collectors.toList());
        female.forEach(System.out::println);

        // Sort
        List<Person> sortedPeople = people.stream().sorted(Comparator.comparing(Person::getGender).thenComparing(Person::getAge).reversed()).collect(Collectors.toList());
        sortedPeople.forEach(System.out::println);

        // All match
        boolean allPeopleAgeGT5 = people.stream().allMatch(person -> person.getAge() > 5);
        System.out.println("[All match]: people age greater than 5 -> " + allPeopleAgeGT5);

        // Any match
        boolean anyPeopleAgeLT20 = people.stream().anyMatch(person -> person.getAge() < 20);
        System.out.println("[Any match]: people age less than 20 -> " + anyPeopleAgeLT20);

        // None match
        String targetPerson = "Tim Antonio";
        boolean isFindPerson = people.stream().noneMatch(person -> person.getName().equals(targetPerson));
        ...
        // Group
        Map<Gender, List<Person>> genderListMap = people.stream().collect(Collectors.groupingBy(Person::getGender));
